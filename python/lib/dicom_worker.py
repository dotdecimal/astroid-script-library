...
# Upload a directory of dicom files into new filesystem_item directory
#   param iam: connection settings (url, user token, and ids for context and realm)
#   param dir_name: complete directory path
# 	returns a study iss ID
def make_filesystem_item_from_dir(iam, dir_name, filtered_upload_file_list=None):
    dl.debug("make_filesystem_item_from_dir")
    dir_id = ""
...
# Convert a filesystem_item ref that is a directory of DICOM files into new rt_study
#   param iam: connection settings (url, user token, and ids for context and realm)
#   param dir_id: complete directory path posted to ISS. Can be gotten using make_filesystem_item_from_dir()
# 	returns a study iss ID
def make_rt_study_from_filesystem_item(iam, dir_id):
...
def make_rt_study_from_dir(iam, dir_name, filtered_upload_file_list=None):
    dl.debug("make_rt_study_from_dir")
    dir_id = make_filesystem_item_from_dir(iam, dir_name, filtered_upload_file_list)
    dl.debug('dir_id')
    dl.debug(dir_id)
    res = thinknode.get_immutable(iam, 'dicom', dir_id)
    dl.debug('Got immutable: ' + str(res))
...
def get_dicom_object_ids(iam, list_id):
    ...
    while not_end:
        dd = thinknode.do_calc_array_item(iam, dd_index, thinknode.schema_named_type("dosimetry", "dicom_object"),
                                          list_id, True, False, True)
        dl.debug('dd: ' + dd)
        if 'failed' in dd:
            not_end = False
            break
...
def get_property_array_item_ids(iam, list_id, type_name):
    ...
    not_end = True
    while not_end:
        dd = thinknode.do_calc_array_item(iam, dd_index, thinknode.schema_named_type("dosimetry", type_name), list_id,
                                          True, False, True)
        dl.debug('dd: ' + dd)
        if 'failed' in dd:
            not_end = False
...
def get_plan(iam, patient_id):
    dl.debug("get_plan")
    dd_array = \
        thinknode.do_calc_item_property(iam, 'patient',
            thinknode.schema_array_named_type("dosimetry", "dicom_data"), patient_id)
    ...
    while not_found:
        ...
        dd_obj = thinknode.do_calc_item_property(iam, 'dicom_obj',
...
def get_beam_geometry(iam, study_id, beam_index):
    ...
    couch_angle = \
        thinknode.do_calc_item_property(iam, 'patient_support_angle',
    ...
    patient_position = \
        thinknode.do_calc_item_property(iam, 'position', thinknode.schema_named_type("dosimetry", 'patient_position_type'), setup)
...
def get_pbs_layers(iam, pbs_machine, spots, beam_id):
    ...
    gantry_angle = \
        thinknode.do_calc_item_property(iam, 'gantry_angle',
            thinknode.schema_standard_type("dosimetry", "float_type"),
            control_pt_id)

    layers_calc = \
        thinknode.function(iam["account_name"], "dosimetry", "create_pbs_layers_from_spots",
            [
...
def get_first_control_point_from_beam(iam, beam_id):
    ...
    control_pts = thinknode.do_calc_item_property(iam, 'control_points',
        thinknode.schema_array_named_type("dosimetry", "rt_control_point"),
        beam_id)

    control_pt_id = \
        thinknode.do_calc_array_item(iam, 0, thinknode.schema_named_type("dosimetry", "rt_control_point"), control_pts)
...
def get_range_compensator_from_beam(iam, plan_id, beam_index):
    ...
    beam_array = \
        thinknode.do_calc_item_property(iam, 'beams',
             thinknode.schema_array_named_type("dosimetry", "rt_ion_beam"), plan_id)

    beam_id = \
        thinknode.do_calc_array_item(iam, beam_index, thinknode.schema_named_type("dosimetry", "rt_ion_beam"),
            beam_array)

    rt_rcs = \
        thinknode.do_calc_item_property(iam, 'degraders',
            thinknode.schema_array_named_type("dosimetry", 'rt_ion_rangecompensator'),
            beam_id)
    rt_rc = \
        thinknode.do_calc_array_item(iam, 0, thinknode.schema_named_type("dosimetry", 'rt_ion_rangecompensator'), rt_rcs)
    return rt_rc
...
def get_patient_position(iam, plan_id):
    ...
    setups_array = \
        thinknode.do_calc_item_property(iam, 'patient_setups',
            thinknode.schema_array_named_type("dosimetry", 'rt_patient_setup'),
            plan_id)
    setup = \
        thinknode.do_calc_array_item(iam, 0, thinknode.schema_named_type("dosimetry", 'rt_patient_setup'), setups_array)
...
def get_structure_set(iam, patient_id):
    ...
    dd_array = \
        thinknode.do_calc_item_property(iam, 'patient', thinknode.schema_array_named_type("dosimetry", "dicom_data"), patient_id)
    dl.debug('dd_array: ' + dd_array)
    ...
    while not_found:
        ...
        dd_obj = \
            thinknode.do_calc_item_property(iam, 'dicom_obj', thinknode.schema_named_type("dosimetry", "dicom_object"), dd)
        dl.debug('dd_obj: ' + dd_obj)

        ss = \
            thinknode.do_calc_item_property(iam, 'structure_set',  thinknode.schema_named_type("dosimetry", "rt_structure_set"), dd_obj)
        if not ss:
...
def get_structure_by_index(iam, ss_id, index):
    ...
    structures = \
        thinknode.do_calc_item_property(iam, 'structures', thinknode.schema_array_named_type("dosimetry", "rt_structure"), ss_id)
    structure = \
        thinknode.do_calc_array_item(iam, index, thinknode.schema_named_type("dosimetry", "rt_structure"), structures)
...
#   param iam: connection settings (url, user token, and ids for context and realm)
#	param study_id: rt_study iss id
def get_stopping_power_img(iam, study_id, curve):
    dl.debug("get_stopping_power_img")
    ct_img_data = \
        thinknode.do_calc_item_property(iam, 'ct', thinknode.schema_named_type("dosimetry", "ct_image"), study_id)
    ct_img = \
        thinknode.do_calc_item_property(iam, 'image_set', thinknode.schema_named_type("dosimetry", "ct_image_set"), ct_img_data)
...
def get_ct_image_set(iam, patient_id):
    ...
    while not_found:
        dd = \
            thinknode.do_calc_array_item(iam, dd_index, thinknode.schema_named_type("dosimetry", "dicom_data"), dd_array)
        dl.debug('dd: ' + dd)
...
def structure_geometry_refs(iam, dicom_obj_id):
    ...
    ss_id = \
        thinknode.do_calc_item_property(iam, 'structure_set',
            thinknode.schema_named_type("dosimetry", "rt_structure_set"), dicom_obj_id)
    ss_list_id = \
        thinknode.do_calc_item_property(iam, 'structures',
            thinknode.schema_named_type("dosimetry", "rt_structure_list"), ss_id)
    structure_list_id = \
        thinknode.do_calc_item_property(iam, 'structure_list',
            thinknode.schema_array_named_type("dosimetry", "rt_structure"),
            ss_list_id)
    ...
    for ref in list_of_refs:
        ...
        list_of_ref_iss_ids.append(
            json.loads(thinknode.post_immutable_named(iam, "dosimetry", ref_data, 'rt_structure').text)['id'])

    obj['structure_set']['structures']['ref_structure_list'] = list_of_ref_iss_ids
    del obj['structure_set']['structures']['structure_list']

    ss_dicom_obj_ref_id = json.loads(thinknode.post_immutable_named(iam, "dosimetry", obj, 'dicom_object', True).text)[
        'id']
...
def calc_image_slice_item(index, ct_img, iam, image_slices_array_id):
    ct_image_slice = \
        thinknode.do_calc_array_item(iam, index,
            thinknode.schema_named_type("dosimetry", "ct_image_slice"),
            image_slices_array_id)
    ct_slice = \
        thinknode.do_calc_item_property(iam, 'slice',
            thinknode.schema_named_type("dosimetry", "ct_image_slice_content"), ct_image_slice)
    content = \
        thinknode.do_calc_item_property(iam, 'content',
            thinknode.schema_named_type("dosimetry", "ct_image_slice_data"), ct_slice)
...
def ct_slices_with_ref_data(dicom_obj_id, iam):
    ...
    obj_id = thinknode.do_calc_item_property(iam, 'ct_image', thinknode.schema_named_type("dosimetry", "ct_image"),
                                             dicom_obj_id)
    print('obj_id: ' + obj_id)
    image_slices_array = \
        thinknode.do_calc_item_property(iam, 'image_slices',
            thinknode.schema_array_named_type("dosimetry", "ct_image_slice"), obj_id)
    print(image_slices_array)

    image_slices = Parallel(n_jobs=10, backend="threading")(
        map(delayed(
            functools.partial(calc_image_slice_item, iam=iam, image_slices_array_id=image_slices_array, ct_img=obj)),
            range(len(obj['ct_image']['image_slices']))))

    obj['ct_image']['image_slices'] = image_slices

    # print(str(obj))
    dicom_obj_ref_id = json.loads(thinknode.post_immutable_named(iam, "dosimetry", obj, 'dicom_object', True).text)[
        'id']
...
def calc_dicom_rks_list_item(dicom_id, iam):
    ...
    if dicom_metadata['modality'] == 'ct':
        ...
        rks_id = rks.write_rks_entry_isspost(iam, "imported_dicom_file", "dicom_data", dicom_metadata['series_uid'],
                                             dicom_data, None, 'planning')
        print('rks_id: ' + rks_id)
        ...
        ss_id = thinknode.do_calc_item_property(iam, 'structure_set',
                                                thinknode.schema_named_type("dosimetry", "rt_structure_set"), dicom_id)
        sop_id = thinknode.do_calc_item_property(iam, 'sop_data',
                                                 thinknode.schema_named_type("dosimetry", "dicom_sop_common"), ss_id)
        instance_uid = thinknode.do_calc_item_property(iam, 'instance_uid',
                                                       thinknode.schema_standard_type("string_type"), sop_id, True,
                                                       False, True)
        rks_id = rks.write_rks_entry_isspost(iam, "imported_dicom_file", "dicom_data", instance_uid, dicom_data, None,
                                             'planning')
        print('rks_id: ' + rks_id)
    else:
